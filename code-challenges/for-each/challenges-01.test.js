...
const fizzbuzz = (arr) => {
  ...
  arr.forEach(number => {
    if(divThree(number) && divFive(number)) {
      output.push('Fizz Buzz');
    } else {
      if(divThree(number)) output.push('Fizz');
      if(divFive(number)) output.push('Buzz');
      if(!divThree(number) && !divFive(number)) output.push(number);
    }
