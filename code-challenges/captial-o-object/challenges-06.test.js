...
const getHouses = (arr) => {
  let houses = [];
  Object.values(arr).forEach(value => houses.push(value.house));
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  Object.values(arr).forEach(val => {
    if(val.name === character) hasChildren = val.children.length !== 0;
  });
  return hasChildren;
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  Object.entries(arr).forEach(entry => {
    if(entry[1]['name'] === character && entry[1]['children'].length !== 0) hasChildren = true;
  });
  return hasChildren;
};
