...
const forLoopTwoToThe = arr => {
  for (let i in arr) arr[i] = Math.pow(2, arr[i]);
  return arr;
};
...
const forEachTwoToThe = arr => {
  arr.forEach((itm, idx) => (arr[idx] = Math.pow(2, arr[idx])));
  return arr;
};
...
const charCode = arr => arr.map(x => x.charCodeAt());
...
const evenOdd = arr =>
  arr.map(x =>
    !parseInt(x) ? (x = "N/A") : x % 2 === 0 ? (x = "even") : (x = "odd")
  );
...
const extractStats = arr => {
  return arr.map(val => {
    val = { name: val.name, total: val.effort + val.baseStat };
  });
};
